...
*/
static int	key_newserver(t_config *config, char *key, char *value,
			      const char *filename, unsigned int lineno)
{
  ...
  if (*key)
    {
      fprintf(stderr, "%s:%u: warning: subkey in newserver key.\n",
	      filename, lineno);
      return (0);
    }
  if (!*value)
    {
      fprintf(stderr, "%s:%u: warning: No value in newserver key.\n",
	      filename, lineno);
      return (0);
    }
...
*/
static int	process_key_value(t_config *config, char *key, char *value,
				  const char *filename, unsigned int lineno)
{
  ...
  fprintf(stderr, "%s:%u: warning: Unknown key `%s`.\n",
	  filename, lineno, firstkey);
  return (0);
}
...
*/
static int	process_line(t_config *config, char *line,
			     const char *filename, unsigned int lineno)
{
  char		*key;
  char		*value;
