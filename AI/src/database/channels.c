                                       ...
                                       const char* chan)
{
  ...
  ret = asprintf(&req,
                 "SELECT * FROM " TABLE_CHANS
                 " WHERE "
                 "channel='%s' AND server='%s';",
                 echan,
                 eserv);
  free(eserv);
  free(echan);
...
t_id database_insert_chan(t_db* db, const char* serv, const char* chan)
{
  ...
  ret = asprintf(&req,
                 "INSERT INTO " TABLE_CHANS
                 " (channel, server) VALUES "
                 "('%s', '%s');",
                 echan,
                 eserv);
  free(eserv);
  ...
  if (ret == -1)
    return (0);
  ret =
      sqlite3_exec(db->handler, req, &database_callback_nothing, NULL, &echan);
  if (ret != SQLITE_OK)
  {
    sqlite3_free(echan);
    return (0);
