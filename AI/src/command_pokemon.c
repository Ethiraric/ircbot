...
{
  return (
      luneth_msg(luneth->pk.co, luneth, luneth->pk.chan, luneth->pk.question));
}

int pkq_show_hint(t_luneth* luneth)
{
  return (luneth_msgf(luneth->pk.co,
                      luneth,
                      luneth->pk.chan,
                      "Pokemon hint : %s",
                      luneth->pk.hint));
...
{
  static const char* msg = "pokemon ( on | off )";

  return (luneth_respond_msg(co, luneth, msg));
}

int command_pokemon(t_bot* bot, t_ircconnection* co, t_luneth* luneth)
{
  char* command;
