...
int pkq_pfrname(t_luneth* luneth)
{
  ...
  ret = asprintf(&luneth->pk.question,
                 "What is the french name of %s ?",
...
int pkq_ptype(t_luneth* luneth)
{
  ...
  ret = asprintf(
      &luneth->pk.question, "What is/are the type(s) of %s ?", poke->name);
  if (poke->type2)
  {
    ret = asprintf(&luneth->pk.ans,
                   "%s %s",
...
int pkq_atype(t_luneth* luneth)
{
  ...
  num = rand() % (ab_entries) + 1;
  ret = asprintf(
      &luneth->pk.question, "What is the type of %s ?", abilities_db[num].name);
  if (ret == -1)
    return (1);
  ret =
      asprintf(&luneth->pk.ans, "%s", pkq_str_fromtype(abilities_db[num].type));
  if (ret == -1)
  {
    free(luneth->pk.question);
    return (1);
...
int pkq_frnum(t_luneth* luneth)
{
  ...
  poke = &pokemons_db[num];
  ret = asprintf(&luneth->pk.question,
                 "What is the french name of the pokemon"
                 " #%d ?",
                 num);
